...
export class ExecutionService {
    ...
    async startExecution(caseId, data, user) {
    ...
    async markCycleComplete(caseId, user, remarks) {
        const exec = await this.execRepo.findByCaseId(caseId, user.organization_id);
        if (!exec || !exec.is_active)
            throw new AppError('No active execution found', 404);
        // A. Log Payment
        await this.logService.addLog(caseId, {
            log_date: new Date().toISOString(),
    ...
    // 3. Stop Execution
    async stopExecution(caseId, user) {
        const exec = await this.execRepo.findByCaseId(caseId, user.organization_id);
        if (!exec || !exec.is_active)
            throw new AppError('Execution is not active', 400);
        // Soft Stop (Flag only)
    ...
    async updateExecution(caseId, data, user) {
        const exec = await this.execRepo.findByCaseId(caseId, user.organization_id);
        if (!exec)
            throw new AppError('Execution record not found', 404);
    ...
    // NEW: Hard Delete
    async deleteExecution(caseId, user) {
        const exec = await this.execRepo.findByCaseId(caseId, user.organization_id);
        if (!exec)
            throw new AppError('Execution record not found', 404);
