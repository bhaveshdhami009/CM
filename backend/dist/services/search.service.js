...
export class SearchService {
        ...
        if (filters && filters.length > 0) {
            qb.andWhere(new Brackets(qbInner => {
                filters.forEach((filter, index) => {
                    ...
                    if (filter.field === 'court_name')
                        dbField = 'court.name';
                    if (filter.field === 'district_name')
                        dbField = 'district.name';
                    // Always map core identifiers to Case table (c), even if searching Logs/Hearings
                    if (filter.field === 'case_no')
                        dbField = 'c.case_no';
                    if (filter.field === 'file_no')
                        dbField = 'c.file_no';
                    if (filter.field === 'filing_no')
                        dbField = 'c.filing_no';
    ...
    applyCondition(qb, condition, params, index, logic) {
        if (index === 0) {
