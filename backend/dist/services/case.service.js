...
export class CaseService {
    ...
    async getAllCases(user, page, limit, search, filters) {
    ...
    async createCase(data, creator) {
        return AppDataSource.manager.transaction(async (manager) => {
            ...
            if (data.filingDate) {
                ...
                if (!isNaN(d.getTime()))
                    newCase.filing_date = d;
            }
            if (data.receivedDate) {
                const d = new Date(data.receivedDate);
                if (!isNaN(d.getTime()))
                    newCase.received_date = d;
            }
    ...
    async updateCase(id, data, user) {
        ...
        return AppDataSource.manager.transaction(async (manager) => {
            ...
            caseItem.court = { id: data.courtId };
            if (data.establishmentId) {
                caseItem.establishment = { id: data.establishmentId };
            }
            ...
            if (data.filingDate) {
                const d = new Date(data.filingDate);
                if (!isNaN(d.getTime()))
                    caseItem.filing_date = d;
            }
            if (data.receivedDate) {
                const d = new Date(data.receivedDate);
                if (!isNaN(d.getTime()))
                    caseItem.received_date = d;
            }
            ...
            if (data.dlsa && data.dlsa.letterNo) {
                ...
                dlsa.remarks = data.dlsa.remarks;
                if (data.dlsa.letterDate)
                    dlsa.letter_date = new Date(data.dlsa.letterDate);
                if (data.dlsa.billedOn)
                    dlsa.billed_on = new Date(data.dlsa.billedOn);
                if (data.dlsa.receivedOn)
                    dlsa.received_on = new Date(data.dlsa.receivedOn);
                await manager.save(dlsa);
            }
