...
router.patch('/profile', validateDto(UpdateProfileDto, true), // Partial update allowed
controller.updateProfile);
// 2. Admin Routes (Restricted to Org Admin/Editor)
// List Team
router.get('/', hasPermission(ROLES.EDITOR), controller.list);
// Create Member
router.post('/', hasPermission(ROLES.ORG_ADMIN), validateDto(CreateTeamMemberDto), controller.create);
// Remove Member
router.delete('/:id', hasPermission(ROLES.ORG_ADMIN), controller.delete);
// Update Member
router.patch('/:id', hasPermission(ROLES.ORG_ADMIN), validateDto(CreateTeamMemberDto, true), // Partial update allowed
controller.update);
