...
import { Brackets, In, Between } from 'typeorm'; // Import ILike for case-insensitive search
export class CaseRepository {
    constructor() {
        this.repo = AppDataSource.getRepository(Case);
    }
    ...
    async findByReceivedDate(orgId, startDate, endDate) {
    ...
    async findForCalendar(orgId, startDate, endDate) {
        return this.repo.createQueryBuilder('c')
            ...
            .andWhere('(c.filing_date BETWEEN :start AND :end OR c.received_date BETWEEN :start AND :end)', { start: startDate, end: endDate })
            .getMany();
    }
}
