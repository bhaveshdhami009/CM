...
export const processCaseData = (c) => {
    const now = new Date();
    // --- 1. Analyze Hearings ---
    ...
    if (c.hearings && c.hearings.length > 0) {
        ...
        const sortedHearings = c.hearings.sort((a, b) => new Date(a.hearing_date).getTime() - new Date(b.hearing_date).getTime());
        // Find first future hearing
        const next = sortedHearings.find((h) => new Date(h.hearing_date) > now);
        if (next) {
    ...
    // --- 2. Analyze Logs ---
    const pendingLogs = c.logs?.filter((l) => l.is_pending);
    const completedLogs = c.logs?.filter((l) => !l.is_pending)
