...
import { CreateLogDto } from '../dto/log.dto.js'; // Corrected DTO Import
...
export class LogController {
  ...
  update = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      let isPendingBool: boolean | undefined = undefined;
  ...
  togglePending = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      const id = parseInt(req.params.id);
  ...
  getCalendar = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const events = await this.service.getCalendarEvents(req.user!, start as string, end as string);
      res.json(events);
    } catch (err) {
      next(err);
    }
  ...
  delete = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (isNaN(id)) return next(new AppError('Invalid Log ID', 400));
