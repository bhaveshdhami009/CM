...
export class SecurityService {
  ...
  async checkLockdown(email: string) {
    ...
    if (attempt.lockout_until && new Date() < attempt.lockout_until) {
      ...
      if (waitSeconds > 60) timeMsg = `${Math.ceil(waitSeconds / 60)} minutes`;
      if (waitSeconds > 3600) timeMsg = `${Math.ceil(waitSeconds / 3600)} hours`;
