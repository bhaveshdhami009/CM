...
import { CreateGroupDto } from '../dto/chat.dto.js';
...
export class ChatService {
  ...
  private partRepo = new ParticipantRepository();
  ...
  async createDM(identifier: { targetUserId?: number; email?: string }, creator: User) {
    ...
    if (creator.id === targetUser.id) throw new AppError("Cannot chat with yourself", 400);
    ...
    if (!isAdmin) {
      ...
      if (isBlocked || iBlocked) throw new AppError('Cannot start chat. Block restrictions apply.', 403);
    }
  ...
  async sendMessage(convoId: number, content: string | undefined, filePath: string | undefined, user: User) {
    // FIX: Use this.convoRepo
    ...
    if (convo.type === ChatType.GROUP) {
      if (user.role < convo.min_role_write) throw new AppError('Read Only', 403);
    }
  ...
  async markAsRead(convoId: number, userId: number) {
  ...
  async createGroup(data: CreateGroupDto, creator: User) {
    return AppDataSource.manager.transaction(async (manager) => {
      const convo = new ChatConversation();
      ...
      const addParticipant = async (userId: number, isAdmin: boolean) => {
      ...
      for (const admin of orgAdmins) {
          if (admin.id !== creator.id) await addParticipant(admin.id, true);
      }
      ...
      for (const uid of usersToAdd) {
        ...
        if (uid === creator.id || isOrgAdmin) continue; 
  ...
  async respondToInvite(convoId: number, status: 'ACCEPT' | 'REJECT', user: User) {
    const participant = await this.partRepo.findMember(convoId, user.id);

    if (!participant) throw new AppError('Invite not found', 404);
    if (participant.status !== ParticipantStatus.PENDING) throw new AppError('Invite already processed', 400);
  ...
  async addGroupMember(convoId: number, targetUserId: number, actor: User) {
    const convo = await this.convoRepo.findOne({ where: { id: convoId } });
    if (!convo || convo.type !== ChatType.GROUP) throw new AppError('Invalid Group', 400);
    ...
    if (!isOrgAdmin && !isGroupAdmin) throw new AppError('Access Denied', 403);
  ...
  async removeGroupMember(convoId: number, targetUserId: number, actor: User) {
    const convo = await this.convoRepo.findOne({ where: { id: convoId } });
    if (!convo || convo.type !== ChatType.GROUP) throw new AppError('Invalid Group', 400);
    ...
    if (actor.id !== targetUserId) {
        if (!isActorOrgAdmin && !isActorGroupAdmin) throw new AppError('Access Denied', 403);
    }
  ...
  async getMessages(convoId: number, user: User, limit: number = 20, offset: number = 0) {
  ...
  async blockUser(targetId: number, user: User) {
    if (targetId === user.id) throw new AppError("Cannot block yourself", 400);
  ...
  async unblockUser(targetId: number, user: User) {
  ...
  async updateGroup(convoId: number, data: any, user: User) {
    const convo = await this.convoRepo.findOne({ where: { id: convoId } });
    if (!convo) throw new AppError('Conversation not found', 404);
    ...
    if (data.description) convo.description = data.description;
    
    if (data.min_role_read !== undefined) convo.min_role_read = data.min_role_read;
    if (data.min_role_write !== undefined) convo.min_role_write = data.min_role_write;

    return this.convoRepo.save(convo);
  }
  
  async deleteConversation(convoId: number, user: User) {
    const convo = await this.convoRepo.findOne({ where: { id: convoId } });
    if (!convo) throw new AppError('Conversation not found', 404);
  ...
  async deleteMessage(msgId: number, user: User) {
    ...
    if (msg.sender.id !== user.id) throw new AppError('Cannot delete others messages', 403);
  ...
  async getAttachment(msgId: number, user: User) {
    const msg = await this.msgRepo.findOne({ 
        where: { id: msgId },
        relations: { conversation: true }
    });
    if (!msg || !msg.file_path) throw new AppError('File not found', 404);
    ...
    if (!absolutePath.includes('uploads')) throw new AppError('Invalid file path', 400); 

    if (!fs.existsSync(absolutePath)) throw new AppError('File missing on disk', 404);
