...
import { CreateHearingDto } from '../dto/hearing.dto.js'; // Correct DTO path
...
export class HearingService {
  ...
  private async rebuildHearingChain(caseId: number, manager: any) {
  ...
  async addHearing(caseId: number, data: CreateHearingDto, user: User) {
    ...
    if (isNaN(hearingDate.getTime())) throw new AppError('Invalid Hearing Date', 400);
  ...
  async updateHearing(id: number, data: CreateHearingDto, user: User) {
    // Check ownership via Repo
    const hearing = await this.hearingRepo.findById(id, user.organization_id!);
    if (!hearing) throw new AppError('Hearing not found', 404);

    return AppDataSource.manager.transaction(async (manager) => {
      // 1. Update fields
      if (data.hearing_date) hearing.hearing_date = new Date(data.hearing_date);
      if (data.purpose) hearing.purpose = data.purpose;
      
      // Handle optionals: explicitly allow null/undefined updates if passed
      if (data.court_room !== undefined) hearing.court_room = data.court_room;
      if (data.judge_id !== undefined) hearing.judge_id = data.judge_id;
      if (data.outcome !== undefined) hearing.outcome = data.outcome;
      if (data.remarks !== undefined) hearing.remarks = data.remarks;
  ...
  async deleteHearing(id: number, user: User) {
    const hearing = await this.hearingRepo.findById(id, user.organization_id!);
    if (!hearing) throw new AppError('Hearing not found', 404);
  ...
  async getHearings(caseId: number, user: User) {
