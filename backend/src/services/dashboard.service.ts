...
export class DashboardService {
  ...
  private hearingRepo = AppDataSource.getRepository(Hearing);
  private logRepo = AppDataSource.getRepository(CaseLog);

  // 1. STATS (Unchanged)
  async getStats(orgId: number, startStr: string, endStr: string) {
