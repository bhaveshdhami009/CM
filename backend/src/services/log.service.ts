...
import { CaseLog } from '../entities/CaseLog.js'; // Import Entity for typing
import { AppError } from '../utils/AppError.js';
import { CreateLogDto } from '../dto/log.dto.js'; // Ensure correct path
...
export class LogService {
  ...
  private hearingRepo = new HearingRepository();
  private execRepo = new ExecutionRepository();

  async getLogs(caseId: number, user: User) {
  ...
  async addLog(caseId: number, data: CreateLogDto, user: User) {
    const caseExists = await this.caseRepo.findById(caseId, user.organization_id!);
    if (!caseExists) throw new AppError('Case not found', 404);

    // Create Partial<CaseLog> to satisfy TypeORM
    const logData: Partial<CaseLog> = {
      case_id: caseId,
  ...
  async updateLog(id: number, data: CreateLogDto, user: User) {
    ...
    // 3. Update fields
    if (data.log_date) log.log_date = new Date(data.log_date);
    if (data.purpose) log.purpose = data.purpose;
    if (data.remarks !== undefined) log.remarks = data.remarks;
    if (data.is_pending !== undefined) log.is_pending = Boolean(data.is_pending); 
  ...
  async deleteLog(id: number, user: User) {
    // 1. Verify ownership
    const log = await this.logRepo.findById(id, user.organization_id!);
  ...
  private deleteFileFromDisk(relativePath: string) {
    try {
      const absolutePath = path.resolve(relativePath);
      // Security: Prevent Directory Traversal
      if (!absolutePath.includes('uploads')) return; 
  ...
  async getLogFile(caseId: number, logId: number, user: User) {
    const caseExists = await this.caseRepo.findById(caseId, user.organization_id!);
    if (!caseExists) throw new AppError('Case not found or access denied', 404);

    const log = await this.logRepo.findById(logId, user.organization_id!);
    if (!log || log.case_id !== caseId) throw new AppError('Log not found', 404);
    if (!log.file_path) throw new AppError('No file attached', 404);

    const absolutePath = path.resolve(log.file_path);
    if (!absolutePath.includes('uploads')) throw new AppError('Invalid file path', 400);
    if (!fs.existsSync(absolutePath)) throw new AppError('File missing on server', 404);
  ...
  async getCalendarEvents(user: User, start: string, end: string) {
    const startDate = new Date(start);
    const endDate = new Date(end);
    
    if (isNaN(startDate.getTime()) || isNaN(endDate.getTime())) throw new AppError('Invalid date', 400);
  ...
  async togglePendingStatus(logId: number, user: User) {
    const log = await this.logRepo.findById(logId, user.organization_id!);
    if (!log) throw new AppError('Log not found', 404);
