...
import { CreateOrgDto } from '../dto/org.dto.js'; // Ensure correct path
...
export class OrganizationService {
  ...
  async onboardOrganization(data: CreateOrgDto) {
    // 1. Pre-checks (Read-only, no transaction needed yet)
    const existingOrg = await this.orgRepo.findByName(data.name);
    if (existingOrg) throw new AppError('Organization Name already exists', 409);
  ...
  async updateOrganization(id: number, data: Partial<CreateOrgDto>) {
    ...
    if (data.contact_email) org.contact_email = data.contact_email;
